...
RETENTION_DAYS=14    # опційно — через скільки днів авточистка видаляє завершене/минуле
END_REMINDER_MIN=15  # опційно — за скільки хв до кінця події нагадувати (notify_end)
```
...
й є `end_time` — додаткове нагадування за `config.END_REMINDER_MIN` (env, за замовч. 15) хв до кінця (job `endreminder:{id}` поряд з
...
Видаляє завершене/минуле старше за `config.RETENTION_DAYS` (env, за замовч. 14 днів): минулі події та виконані задачі (з датою
